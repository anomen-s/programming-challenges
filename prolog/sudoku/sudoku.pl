% vyreseni sudoku v prologu

% pouziti:
% * vyplnit zname hodnoty
% * volitelne doplnit dalsi omezujici podminky a optimalizace
% ** efektivni hlavne u prvnich trech radku 
% ** vzdy vkladat az za unifikaci vsech potrebnych policek !!!

% optimalizace: 
% * napr. A1=\=6, pokud napr A8=6
% * member(5, [A1, C1]), pokud vime, ze 5 musi byt v A1, nebo v C1
 
% TIP: otocit / zrcadlit cele sudoku tak, aby bylo v prvnich radcich co nejvice znamych poli
...
row(A1,A2,A3,A4,A5,A6,A7,A8,A9) :- is_set([A1,A2,A3,A4,A5,A6,A7,A8,A9]).

sel(List, Item, Rest) :- select(Item, List, Rest). % too lazy to rewrite...

printboard([A,B,C,D,E,F,G,H,I|R]) :-
    print(A),print(' '),print(B),print(' '),print(C),print(' '),print(D),print(' '),
    print(E),print(' '),print(F),print(' '),print(G),print(' '),print(H),print(' '),
    print(I),nl,printboard(R).
printboard([]).

solve(Board) :-
    numbers(N),

    % known values, eg. B1=1,H5=5,
...
%%%%         RADEK                  3x3 BLOK		 sloupec	constraints & optimalizations
% RADEK1
    sel(N,	 A1,R1C1),	sel(N,	 A1,Q11a),	sel(N,	 A1,C1C1),    %%%
...
% RADEK-2
    ...
    sel(R2C8,I2,[]),	sel(Q13e,I2,Q13f),	sel(C9C1,I2,C9C2),

% RADEK-3
    sel(N,	 A3,R3C1),	sel(Q11f,A3,Q11g),	sel(C1C2,A3,C1C3),
    sel(R3C1,B3,R3C2),	sel(Q11g,B3,Q11h),	sel(C2C2,B3,C2C3),
    ...
    sel(R3C8,I3,[]),	sel(Q13h,I3,[]),	sel(C9C2,I3,C9C3),
